...
/*
resource "aws_api_gateway_resource" "order_resource_proxy" {
  # 연결된 REST API의 ID
  rest_api_id = aws_api_gateway_rest_api.order_api.id
  # 상위 API 리소스의 ID
...
resource "aws_api_gateway_method" "order_method_proxy" {
  rest_api_id   = aws_api_gateway_rest_api.order_api.id
  resource_id   = aws_api_gateway_resource.order_resource_proxy.id
  http_method   = "ANY"
  authorization = "NONE"
}

resource "aws_api_gateway_integration" "order_proxy_integration" {
  rest_api_id             = aws_api_gateway_rest_api.order_api.id
  resource_id             = aws_api_gateway_resource.order_resource_proxy.id
  # 연결된 리소스를 호출할 떄
  http_method             = aws_api_gateway_method.order_method_proxy.http_method
  
  # 백엔드를 호출할때 이용
  # 람다 기능은 POST를 통해서만 호출할 수 있습니다.
  integration_http_method = "POST"
  type                    = "AWS_PROXY"
  uri                     = aws_lambda_function.order.invoke_arn
}
